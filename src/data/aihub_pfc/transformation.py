...
class Example:
    ...
    def _create_positive_subsequence(self, substring, label):
        ...
        for idx in range(len(words) - 1): labels.append(''.join([label, self._consecutive]))
        return words, labels

    def _create_negative_subsequence(self, substring, label):
        words = substring.split()
    ...
    def create_sequence(self, example):
        if not example['NE']:
            ...
            substring = example['text'][head_idx:tail_idx]
            subsequence = self._create_subsequence(substring, entities_sorted[-1]['type'])
