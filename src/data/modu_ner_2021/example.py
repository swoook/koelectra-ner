class Example:
    ...
    def convert(self, example):
        ...
        if not example['NE']:
            self._create_subsequence(example['form'], '-')
            ...
            if entities_sorted[0]['begin'] > 0:
                ...
                substring = example['form'][:tail_idx]
                self._create_subsequence(substring, '-')

            for idx, entity in enumerate(entities_sorted[:-1]):
                head_idx = entity['begin']
                tail_idx = entity['end']
                substring = example['form'][head_idx:tail_idx]
                self._create_subsequence(substring, entity['label'][:2])
            ...
            substring = example['form'][head_idx:tail_idx]
            self._create_subsequence(substring, entities_sorted[-1]['label'][:2])

            if entities_sorted[-1]['end'] < len(example['form']):
                head_idx = entities_sorted[-1]['end']
                tail_idx = len(example['form'])
                substring = example['form'][head_idx:tail_idx]
                self._create_subsequence(substring, '-')
                
        assert len(self._word_sequence) == len(self._label_sequence)
